...
| 3 | Service / version detection (`-sV`) | full cycle | 🔶 **CURRENT** — Phase-0 + spike done; port order approved. Modules 1–3/6 DONE. **`core::probedb`**: exact C counts (186+1 probes, 12,171 rules). **`core::pcre_translate`**: 77.50%→**93.57%** linear acceptance. **`core::matcher`**: hybrid `regex::bytes`→`fancy-regex` (backtrack-bounded, latin-1 for binary banners) compiles **all 12,171 rules, 0 dropped**; first-match-wins; ReDoS-immune. All six gates green (0 unsafe, miri clean). Next: `core::versioninfo` (`$1`/`$P` substitution) |
