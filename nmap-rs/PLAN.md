...
| 3 | Service / version detection (`-sV`) | full cycle | 🔶 **CURRENT** — Phase-0 + spike done; port order approved. Module 1/6 **`core::probedb`** DONE (six gates green: parses the real 2.5 MB file to the exact C counts — 186+1 probes, 12,171 rules — with 0 warnings; fuzz 1.14M runs 0 crashes; miri clean; 0 unsafe). Next: `core::pcre_translate` |
