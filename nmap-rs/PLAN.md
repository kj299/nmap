...
| 3 | Service / version detection (`-sV`) | full cycle | 🔶 **CURRENT** — Phase-0 + spike done; port order approved. Modules 1–5/6 DONE. **`core::probedb`** (exact C counts, 12,171 rules) → **`core::pcre_translate`** (77.50%→93.57%) → **`core::matcher`** (hybrid, all 12,171 rules, ReDoS-immune) → **`core::versioninfo`** (`$1`/`$P`/`$SUBST`/`$I`, no fixed buffers) → **`core::servicescan`** + **`sys::servicescan`** (pure `nextProbe` scheduler + tokio driver — connect path; SSH detected end-to-end from a real loopback banner). 0 unsafe, miri clean. Next: `cli` (6/6 — `-sV` flags + VERSION output + differential) |
