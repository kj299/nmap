...
| 3 | Service / version detection (`-sV`) | full cycle | 🔶 **CURRENT** — Phase-0 + spike done; port order approved. Modules 1–2/6 DONE. **`core::probedb`**: parses the real 2.5 MB file to exact C counts (186+1 probes, 12,171 rules), 0 warnings. **`core::pcre_translate`**: PCRE→Rust syntax preprocessor lifts linear-engine acceptance 77.50%→**93.57%** over the 12,171 patterns, no pattern made worse. Both six gates green (fuzz 0 crashes, miri clean, 0 unsafe). Next: `core::matcher` |
